...
void EnemyBossAxe::Init(std::shared_ptr<Physics> physics)
{
...
void EnemyBossAxe::Update(int model, float currentFrame, const AttackTiming& timing)
{
...
void EnemyBossAxe::IdleUpdate(int model)
{
	// 当たり判定を無効化する
	SetActive(false);
...
void EnemyBossAxe::AttackUpdate(int model)
{
	// 当たり判定を有効化する
	SetActive(true);
...
void EnemyBossAxe::Draw()
{
	MV1DrawModel(m_model);
}

void EnemyBossAxe::OnCollide(std::shared_ptr<Collidable> collider)
{
}
...
SubEnemyBoss::SubEnemyBoss() :
	m_walkFrame(0.0f),
	SubEnemyBase(ObjectTag::Boss, ObjectPriority::Middle, ColliderData::Kind::Capsule)
{
}
...
void SubEnemyBoss::Init(Vector3& pos, Vector3& rot, Vector3& scale)
{
	// Physicsに登録
	Collidable::Init();
	m_rigidbody.Init();
	...
	// 武器
	m_leftWeapon = std::make_shared<SubEnemyBossLeftWeapon>();
	m_leftWeapon->Init();
	m_rightWeapon = std::make_shared<SubEnemyBossRightWeapon>();
	m_rightWeapon->Init();
}

void SubEnemyBoss::Draw()
{
...
void SubEnemyBoss::Update(std::shared_ptr<SubPlayer> player)
{
	if (m_isDead && m_model < 0)
	{
		return;
...
void SubEnemyBoss::FindUpdate(std::shared_ptr<SubPlayer> player)
{
	SetActive(true);
	m_rightWeapon->Update(m_model, m_atkFrame, BossAtk::kColTimingTable.at(SubEnemyState::Find));
	m_leftWeapon->Update(m_model, m_atkFrame, BossAtk::kColTimingTable.at(SubEnemyState::Find));
...
void SubEnemyBoss::WalkUpdate(std::shared_ptr<SubPlayer> player)
{
	SetActive(false);
	m_rightWeapon->Update(m_model, m_atkFrame, BossAtk::kColTimingTable.at(SubEnemyState::Walk));
	m_leftWeapon->Update(m_model, m_atkFrame, BossAtk::kColTimingTable.at(SubEnemyState::Walk));
	...
	if (m_walkFrame >= 120.0f)
	{
		...
		{
			if (distance > 500.0f && m_prevState != SubEnemyState::Stab)
			{
				ChangeState(SubEnemyState::Stab);
			}
			else if (distance > 400.0f && m_prevState != SubEnemyState::Chop)
			{
				ChangeState(SubEnemyState::Chop);
			}
			else if (distance > 300.0f && m_prevState != SubEnemyState::Slice)
			{
				ChangeState(SubEnemyState::Slice);
			}
			else if (distance < 300.0f && m_prevState == SubEnemyState::Spin)
			{
				ChangeState(SubEnemyState::Slice);
			}
			else if (m_prevState != SubEnemyState::Spin)
			{
				ChangeState(SubEnemyState::Spin);
			}
		}
	}
}

void SubEnemyBoss::ChaseUpdate(std::shared_ptr<SubPlayer> player)
{
	SetActive(true);
	m_rightWeapon->Update(m_model, m_atkFrame, BossAtk::kColTimingTable.at(SubEnemyState::Chase));
	m_leftWeapon->Update(m_model, m_atkFrame, BossAtk::kColTimingTable.at(SubEnemyState::Chase));
...
void SubEnemyBoss::AttackUpdate(std::shared_ptr<SubPlayer> player)
{
	SetActive(true);
	++m_atkFrame;
	m_rightWeapon->Update(m_model, m_atkFrame, BossAtk::kColTimingTable.at(SubEnemyState::Attack));
	m_leftWeapon->Update(m_model, m_atkFrame, BossAtk::kColTimingTable.at(SubEnemyState::Attack));
...
void SubEnemyBoss::ChopUpdate(std::shared_ptr<SubPlayer> player)
{
	SetActive(true);
	++m_atkFrame;
	m_rightWeapon->Update(m_model, m_atkFrame, BossAtk::kColTimingTable.at(SubEnemyState::Chop));
	m_leftWeapon->Update(m_model, m_atkFrame, BossAtk::kColTimingTable.at(SubEnemyState::Chop));
...
void SubEnemyBoss::SliceUpdate(std::shared_ptr<SubPlayer> player)
{
	SetActive(true);
	++m_atkFrame;
	m_rightWeapon->Update(m_model, m_atkFrame, BossAtk::kColTimingTable.at(SubEnemyState::Slice));
	m_leftWeapon->Update(m_model, m_atkFrame, BossAtk::kColTimingTable.at(SubEnemyState::Slice));
...
void SubEnemyBoss::StabUpdate(std::shared_ptr<SubPlayer> player)
{
	SetActive(true);
	++m_atkFrame;
	m_rightWeapon->Update(m_model, m_atkFrame, BossAtk::kColTimingTable.at(SubEnemyState::Stab));
	m_leftWeapon->Update(m_model, m_atkFrame, BossAtk::kColTimingTable.at(SubEnemyState::Stab));
...
void SubEnemyBoss::SpinUpdate(std::shared_ptr<SubPlayer> player)
{
	SetActive(true);
	++m_atkFrame;
	m_rightWeapon->Update(m_model, m_atkFrame, BossAtk::kColTimingTable.at(SubEnemyState::Spin));
	m_leftWeapon->Update(m_model, m_atkFrame, BossAtk::kColTimingTable.at(SubEnemyState::Spin));
...
void SubEnemyBoss::HitUpdate(std::shared_ptr<SubPlayer> player)
{
	SetActive(false);
	m_rightWeapon->Update(m_model, m_atkFrame, BossAtk::kColTimingTable.at(SubEnemyState::Hit));
	m_leftWeapon->Update(m_model, m_atkFrame, BossAtk::kColTimingTable.at(SubEnemyState::Hit));
...
void SubEnemyBoss::DeadUpdate(std::shared_ptr<SubPlayer> player)
{
	SetActive(false);
	m_rightWeapon->Update(m_model, m_atkFrame, BossAtk::kColTimingTable.at(SubEnemyState::Dead));
	m_leftWeapon->Update(m_model, m_atkFrame, BossAtk::kColTimingTable.at(SubEnemyState::Dead));
...
const char* SubEnemyBoss::GetAnimName(SubEnemyState state) const
{
	switch (state)
	{
	case SubEnemyState::Walk:
	...
	case SubEnemyState::Chop:
		return kChopAttackAnimName;
	case SubEnemyState::Slice:
		return kSliceAttackAnimName;
	case SubEnemyState::Stab:
		return kStabAttackAnimName;
	case SubEnemyState::Spin:
...
float SubEnemyBoss::GetAnimPlaySpeed(SubEnemyState state) const
{
	switch (state)
	{
	case SubEnemyState::Find:
...
bool SubEnemyBoss::IsLoopAnim(SubEnemyState state) const
{
	switch (state)
	{
	case SubEnemyState::Walk:

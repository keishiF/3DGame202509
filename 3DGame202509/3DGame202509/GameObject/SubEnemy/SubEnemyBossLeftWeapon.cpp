...
SubEnemyBossLeftWeapon::SubEnemyBossLeftWeapon() :
	m_model(-1),
...
void SubEnemyBossLeftWeapon::Init()
{
	Collidable::Init();
	m_rigidbody.Init();
...
void SubEnemyBossLeftWeapon::Update(int model, float currentFrame, const BossAtk::AtkTiming& timing)
{
...
void SubEnemyBossLeftWeapon::IdleUpdate(int model)
{
	// 当たり判定を無効化する
	SetActive(false);
...
void SubEnemyBossLeftWeapon::AttackUpdate(int model)
{
	// 当たり判定を有効化する
	SetActive(true);
...
void SubEnemyBossLeftWeapon::Draw()
{
	MV1DrawModel(m_model);
}

void SubEnemyBossLeftWeapon::OnCollide(std::shared_ptr<Collidable> collider)
{
}

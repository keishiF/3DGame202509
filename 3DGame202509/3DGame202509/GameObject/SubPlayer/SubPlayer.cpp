...
void SubPlayer::IdleUpdate()
{
	...
	if (input.IsTrigger("X"))
	{
		if (m_stamina >= kSpinStamina)
		{
			...
			m_staminaRate = m_stamina / kStamina;
			m_staminaRate = std::clamp(m_staminaRate, 0.0f, 1.0f);
			ChangeState(SubPlayerState::Spin);
		}
	}
...
void SubPlayer::WalkUpdate()
{
	...
	if (input.IsTrigger("RB"))
	{
		...
		{
			ChangeState(SubPlayerState::NormalRun);
		}
		else
		{
...
void SubPlayer::SpinUpdate()
{
	...
	++m_atkFrame;
	m_weapon->Update(m_model, m_atkFrame, kColTimingTable.at(SubPlayerState::Spin), false);
...
void SubPlayer::SpecialUpdate()
{
	++m_atkFrame;
	SetActive(false);
	m_weapon->Update(m_model, m_atkFrame, kColTimingTable.at(SubPlayerState::Special), true);
...
{
	SetActive(false);
	m_weapon->Update(m_model, m_atkFrame, kColTimingTable.at(SubPlayerState::Hit), false);
...
void SubPlayer::SetSpecialGauge(int specialGaugePoint)
{
